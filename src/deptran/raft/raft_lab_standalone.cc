...
bool case_disconnect_blocks_one_node(TestCluster& c) {
  c.reset_faults();
  c.disconnect(2);
  // Use a valid vote request: a real server correctly rejects an empty
  // TimeoutNow, which made the old dummy-dispatcher assertion meaningless.
  auto dropped = c.node(1).transport()->send_vote(2, VoteReq{2, 0, 1, 1});
  auto ok3 = c.node(1).transport()->send_vote(3, VoteReq{2, 0, 1, 1});
  c.reset_faults();
